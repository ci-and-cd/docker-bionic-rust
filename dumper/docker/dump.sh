...
for layer in $(pwd)/data/image/*/layer.tar; do
    ...
    if [ -n "$(tar -tf ${layer} | grep 'cargo/bin/rustup')" ]; then echo found cargo/bin/rustup in ${layer}; LAYERS+=(${layer}); fi
done
...
tar_directories=($(tar tf data/layer.tar | grep -E '.*/$' | grep -v 'rust' | sort -r -n))
